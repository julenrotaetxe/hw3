...
ublurNeuBC = reshape(v, n, n);


figure(1); clf;
subplot(2, 2, 1);pcolor(u); 
title('pcolor of the original image');% try this one too
caxis([0 1]);colormap(gray);axis equal, axis tight;
subplot(2, 2, 2);imagesc(u);
title('imagesc of the original image');
caxis([0 1]);colormap(gray);axis equal, axis tight;
subplot(2,2,3);imagesc(ublur);title('Blur image. Dirichlet BC');
caxis([0 1]);colormap(gray);axis equal, axis tight;
subplot(2,2,4);imagesc(ublurNeuBC);title('Blur image. Neumann BC');
caxis([0 1]);colormap(gray);axis equal, axis tight;
